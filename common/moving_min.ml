...
type t = { h : Sample.t H.t; q : Sample.t H.Elt.t Q.t; window : float }
[@@deriving sexp_of]
...
let cleanup t ~param =
  let rec loop () =
    ...
    | Some elt ->
        ...
        if Float.(sample.param + t.window < param) then (
...
let get t =
  match H.top t.h with None -> None | Some sample -> Some sample.value
