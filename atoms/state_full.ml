...
let most_recent t = Global.get t.global t.most_recent
...
let rec render_loop t =
  ...
  Ctx.clear t.ctx;
  Global.iter t.global ~f:(fun box -> Atom.render box t.ctx ~time);
  if t.is_server
  then (
...
let on_box_active t ~f = t.on_box_active <- f :: t.on_box_active
...
let create ctx ~is_server =
  ...
  in
  Global.create_exn global_config ~sexp_of_a:Atom.sexp_of_t
  >>= fun global ->
  let t =
    { ctx
    ; global
    ; is_server (* Start with a bit of lies, never lie after that. *)
  ...
  if t.is_server
  then
    Global.on_change t.global ~f:(fun _box_id box ->
...
let set_shape t shape = t.template <- Box.set t.template ~kind:shape
let toggle_transient_mode t = t.transient_mode <- not t.transient_mode
