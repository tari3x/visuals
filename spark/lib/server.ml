...
let get_clicks pixi =
  (* NB! We assume that mouse click coordinates are the same as canvas
     coordinates. *)
  let clicks = Pixi.actions pixi |> Lwt_stream.filter_map ~f:Action.click in
...
let _test_quantum () =
  ...
  let rec loop () =
    ...
    Int.incr count;
    if Time.(Span.(now () - !start > Time.Span.of_sec 1.)) then (
...
let main (config : Config.t) =
  ...
  *)
  ...
  if config.debug_sound then failwith "Fix debug sound"
    (*
    ...
    *)
    ...
    let shapess =
      List.map config.sparks ~f:(function
        | Grid { skin; rows; cols } -> (Shapes.grid_exn ~pixi ~cols ~rows, skin)
        ...
        | Free skin ->
            ...
            let { Svg.shapes; calibration_points; step } = Svg.parse_exn svg in
            let corners =
              match config.calibration with
