...
module Elt = struct
  ...
  type t = { id : Id.t; shape : Shape.t; mutable color : Color_flow.t option }
...
let create ~(config : Config.t) ~pixi ~sound ~(shapes : Shapes.t) ?real_corners
    () =
  ...
  debug [%message "got step"];
  let skin = Skin.Elts.create_exn elts ~step |> Skin.start ~config ~sound in
  debug [%message "got skin"];
