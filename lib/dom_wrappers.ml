...
let is_inside v (target : #Html.element Js.t) =
  ...
  0. <= x
  && x <= float target##.clientWidth
  && 0. <= y
  && y <= float target##.clientHeight
;;
...
module Mouse_event = struct
  ...
  *)
  ...
  let action (target : #Html.element Js.t) (t : t) kind =
    ...
    let changed_touches =
      if not (is_inside position target)
      then []
...
module Touch_event = struct
  ...
  let action (target : #Html.element Js.t) (t : t) kind =
    ...
    let changed_touches =
      List.init changed_touches##.length ~f:(fun i ->
          changed_touches##item i)
...
module Canvas = struct
  ...
  let width t = Float.of_int t##.width
  let height t = Float.of_int t##.height
end
...
module Ctx = struct
  ...
  let canvas_actions t = actions t##.canvas
  let width t = t##.canvas##.clientWidth |> float
  let height t = t##.canvas##.clientHeight |> float
  let clear t = t##clearRect 0. 0. (width t) (height t)
  ...
  let set_line_width (t : t) width = t##.lineWidth := width
  ...
  let set_font t font = t##.font := string font
  let fill_text (t : t) text x y = t##fillText (string text) x y
  let stroke_text (t : t) text x y = t##strokeText (string text) x y
  let save (t : t) = t##save
  ...
  let set_transform (t : t) m =
    ...
    let c i j = m.(i).(j) in
    t##setTransform (c 0 0) (c 1 0) (c 0 1) (c 1 1) (c 0 2) (c 1 2)
  ;;
  ...
  let transform (t : t) m =
    ...
    let m = Matrix.coeffs m in
    let c i j = m.(i).(j) in
    t##transform (c 0 0) (c 1 0) (c 0 1) (c 1 1) (c 0 2) (c 1 2)
  ;;
  ...
  let draw_centered_rectangle (t : t) ~width ~height =
    t##fillRect (-.width /. 2.) (-.height /. 2.) width height
  ;;
...
module Video = struct
  ...
  let create ~id = get_element_by_id id Html.CoerceTo.video
  let read_camera t = Js_std.read_camera t
...
module Audio = struct
  ...
  let create ~id = get_element_by_id id Html.CoerceTo.audio
end
...
module Node_list = struct
  ...
  let to_list (t : 'a t) =
    List.init t##.length ~f:(fun i ->
        Opt.value_exn (t##item i) ~here:[%here])
  ;;
end
