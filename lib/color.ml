...
let of_hex8_string s =
  ...
  in
  { r = get 3; g = get 5; b = get 7; a = Float.of_int (get 1) /. 255. }
;;
...
let rec random_interesting () =
  ...
  in
  let t = random () in
  if interesting t then t else random_interesting ()
;;
...
let interpolate ts ~arg:fraction =
  ...
  | ts ->
    if Float.(fraction <= 0.)
    then List.hd_exn ts
    else if Float.(fraction >= 1.)
    then List.last_exn ts
    else (
...
let set_alpha t ~alpha = { t with a = alpha }
