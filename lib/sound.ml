...
module Source = struct
  ...
  let in_beat t = Beat_detector.in_beat t.beat
...
let num_bins t = (t.bins)##.length
let bin_volume_exn t i = float (Typed_array.unsafe_get t.bins i) /. 256.
...
let find_new_source t ~sources =
  ...
  then None
    ...
    match Array.count t.beats ~f:Beat_detector.in_beat with
    | 0 -> None
    | num_beats ->
...
let draw t ctx =
  ...
  let max_freq =
    Freq.of_hertz ((t.analyser)##.context##.sampleRate / 2.)
  in
  ...
  let bin_w i =
    ...
    (right - left) * spectrum_w / Freq.mel max_freq |> Float.max 1.
  in
  let cursor = ref 0. in
  let draw_bin ~value ~width ?ewma ?wave ?pseudo_min () =
    let height = value * ctx_h * 1. in
    ...
    Option.iter pseudo_min ~f:(draw_line ~color:Color.red);
    cursor := !cursor + width
  in
  Array.iteri t.beats ~f:(fun i beat ->
      ...
      let ewma = None
      (* Beat_detector.Debug.ewma beat |> Option.value ~default:0.6 *)
      in
      let wave =
        Beat_detector.Debug.wave beat |> Option.value ~default:0.5
      in
      (* let wave = if wave > ewma then Some wave else None in *)
      let pseudo_min =
        Beat_detector.Debug.pseudo_min beat
      in
      draw_bin ~value ~width ?ewma ~wave ~pseudo_min () );
...
let create_from_src ~ctx ~src ~max_sources =
  ...
  let beats =
    Array.init num_bins ~f:(fun _i -> Beat_detector.create ())
  in
  { analyser
  ; bins
  ; beats
