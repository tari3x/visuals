...
module Quad = struct
  type t = Vector.t * Vector.t * Vector.t * Vector.t [@@deriving sexp]
  ...
  let geometry (u0, u1, u2, u3) ~canvas =
    ...
    Geometry.create
      ...
      ~vertices
      ~faces:
        [| Face.create (0, 1, 2) ~uvs:(uv0, uv1, uv2)
...
module Surface = struct
  ...
  let create ~canvas ~camera =
    ...
    in
    let x = Math.lusolve a b |> Math.flatten |> Math.Vector.to_array in
  ...
  let draw_camera_image_on_canvas
    ...
    *)
    Object3D.set_matrix
      camera_on_canvas_mesh
      (Matrix4.of_matrix t.camera_to_canvas);
...
let draw_camera_image_on_canvas t ~texture ~scene ~canvas =
  List.iter
    t
    ~f:(Surface.draw_camera_image_on_canvas ~texture ~scene ~canvas)
;;

let draw_border t ~ctx = List.iter t ~f:(Surface.draw_border ~ctx)
