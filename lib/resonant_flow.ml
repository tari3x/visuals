...
module Period = struct
  ...
  let create
    ...
    { span; start; d; value_mult; sin_mult = min_v }
  ;;

  let elapsed t = Time.(now () > add t.start t.span)
  ...
  let eval t =
    ...
    let phase =
      2.
      * pi
      * Time.Span.to_sec Time.(diff (now ()) t.start)
    ...
    let result = t.value_mult * sqrt (int_pow y 2 + int_pow (x - t.d) 2) in
    (* debug "%f" result; *)
    result
  ;;
end
...
let create_exn ~period_range ~value_range =
  ...
  { period_range; value_range; period }
;;
