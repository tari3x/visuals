...
module Box_info : sig
  ...
  let set t ?(box = t.box) ?(owner = t.owner) ?(requested = t.requested) ()
    =
    let last_touched = Time.now () in
...
let get t box_id =
  Option.map (Hashtbl.find t.boxes box_id) ~f:(fun box -> Box_info.box box)
;;
...
let encode_box t box = Box.to_local box ~viewport_scale:t.viewport_scale
let decode_box t box = Box.of_local box ~viewport_scale:t.viewport_scale
...
let init t msg =
  ...
  t.server_offset <- Time.(diff time (now ()))
;;
...
let assign t client_id box_id =
  ...
  in
  match Hashtbl.find t.boxes box_id with
  | None -> if Client_id.equal t.client_id client_id then release ()
  | Some box ->
    if Client_id.equal t.client_id client_id
       && not (Box_info.requested box)
    then release ()
    else (
...
let cleanup_boxes t =
  ...
  t.box_ids
    <- List.filter t.box_ids ~f:(fun box_id ->
         let box = Hashtbl.find_exn t.boxes box_id in
         let last_touched = Box_info.last_touched box in
         let age = Time.(diff time last_touched) in
...
let change t box_id ~f =
  match Hashtbl.find t.boxes box_id with
  | None ->
    error "box not found in change";
    ()
  | Some box -> Hashtbl.set t.boxes ~key:box_id ~data:(f box)
;;
...
let process_message t = function
  ...
  | Message.Release box_id ->
    change t box_id ~f:(fun box -> Box_info.set box ~owner:None ())
  | Message.Grant (client_id, box_id) -> assign t client_id box_id
  ...
  | Message.Set (_, box_id, box) ->
    let box = decode_box t box in
    change t box_id ~f:(fun box_info -> Box_info.set box_info ~box ());
    call_on_change t box_id box
  | Message.Delete box_id ->
    Hashtbl.remove t.boxes box_id;
    t.box_ids <- List.delete t.box_ids box_id ~equal:Box_id.equal
  | Message.Request_init (_, channel) ->
    if t.config.is_server then Faye.publish t.faye channel (init_message t)
  | Message.Init msg -> if not t.config.is_server then init t msg
...
let create_exn config ~sexp_of_a =
  let faye = Faye.create_exn ~sexp_of_a:(Message.sexp_of_t sexp_of_a) in
  let t =
    { config
    ; faye
...
let release t box_id =
  change t box_id ~f:(fun box -> Box_info.set box ~requested:false ());
  if is_owner t box_id then publish t (Message.Release box_id)
;;
...
let delete t box_id =
  if is_owner t box_id then publish t (Message.Delete box_id)
;;

let now_on_server t = Time.(sub (now ()) t.server_offset)
let on_change t ~f = t.on_change <- f :: t.on_change
