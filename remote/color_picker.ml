...
module Config = struct
  type t = { (* included in drawing mode *)
...
let create config ctx = { config; ctx }
...
let color_for_coordinate t =
  ...
  let black_strip_width =
    if t.config.include_black_strip then 30. else 0.
  in
  fun ~width ~height v ->
...
let draw t =
  ...
  for i = 0 to int width do
    for j = 0 to int height do
      if Stdlib.(i mod cell_size) = 0 && Stdlib.(j mod cell_size) = 0
      then (
        ...
        Ctx.fill_rect
          t.ctx
          v
          ~width:cell_size_float
          ~height:cell_size_float)
    done
  done
;;
