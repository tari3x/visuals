...
let color t = Color_cycle.nth_defaulting_to_last_or_white t.color_cycle 0
...
let default kind = create kind

let set_alpha t ~alpha =
  set t ~color_cycle:(Color_cycle.set_alpha (color_cycle t) ~alpha)
;;

let set_translation t v = set t ~frame:(Frame.set_translation (frame t) v)
...
let to_local t ~viewport_scale = { Local.box = t; scale = viewport_scale }
...
let map t ~f = { t with kind = f t.kind }
