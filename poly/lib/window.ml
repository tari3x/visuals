...
module State = struct
  type t =
    { zeroes : V.t list
    ; nonzero : V.t (* all but the last zero *)
    ; lagrange_base : L.t
    ; poly : P.t
    ; prev : t
    }
  ...
  module Update = struct
    ...
    let animation_vector state t : Animation.t =
      match t with
      | Undo | Finish -> None
      | Drop_zero | Add_zero _ | Rotate_zeroes -> Interpolate_poly
      | Move_nonzero v -> Move_point_by V.(v - state.nonzero)
      | Move_zero v ->
        (match state.zeroes with
        | [] -> None
        | v' :: _ -> Move_point_by V.(v - v'))
    ;;
  end

  let active_zero t = List.hd t.zeroes

  let make_lagrange_base ~zeroes ~nonzero =
    let zeroes =
      match zeroes with
      | [] -> []
      | _ :: zeroes -> List.map zeroes ~f:(fun v -> v, 0.)
    in
  ...
  let create ?(zeroes = []) () =
    ...
    let rec t = { zeroes; nonzero; lagrange_base; poly; prev = t } in
    t
  ;;
  ...
  let update t (update : Update.t) =
    ...
    | Move_zero v ->
      ...
      in
      let poly = make_poly ~zeroes ~lagrange_base in
      { zeroes; nonzero; lagrange_base; poly; prev }
    | Move_nonzero v ->
      let lagrange_base = make_lagrange_base ~zeroes ~nonzero in
      let poly = make_poly ~zeroes ~lagrange_base in
      { zeroes; nonzero = v; lagrange_base; poly; prev }
    | Rotate_zeroes ->
  ...
  let point_weighted_average ~w t1 t2 =
    ...
    let { zeroes = zs1
      ...
      t1
    in
    let { zeroes = zs2; nonzero = nz2; lagrange_base; prev; poly = _ } =
      t2
    in
...
let random_point_not_too_close v d =
  ...
  let rec loop () =
    incr count;
    if !count > 1000 then failwith "too many loops of random point";
    let v' = random_point () in
    if Float.(V.(length (v - v')) >= d) then v' else loop ()
  in
  loop ()
;;
...
module Auto_events : Events = struct
  ...
  let create () =
    { updates = Q.create (); count = 0; next_zero_action = `remove }
  ;;
...
module Ctx = struct
  ...
  let record_frame t palette p =
    ...
    t.recording
      <- Option.map t.recording ~f:(fun recording ->
             let state =
  ...
  let write_animation_exn t file =
    ...
    | Some recording ->
      let recording =
        { recording with states = List.rev recording.states }
      in
    ...
    | Some update ->
      ...
      let states =
        ...
        | Interpolate_poly ->
          Motion.smooth_speed
            ~point:(fun w ->
              State.poly_weighted_average state new_state ~w)
            ~distance:State.distance
            ~max_distance
            ~desired_step_size:1.
        | Move_point_by v ->
          let absolute_desired_step_size =
            Float.(0.2 * Config.speed config)
          in
          let desired_step_size =
            absolute_desired_step_size /. V.length v
          in
          Motion.smooth_speed
            ~point:(fun w ->
              State.point_weighted_average state new_state ~w)
            ~distance:State.distance
            ~max_distance
            ~desired_step_size
      in
...
let command =
  ...
  Command.async
    ...
    [%map_open
      let record_to =
        flag "-record-to" (optional Filename.arg_type) ~doc:""
      and write_to =
        flag "-write-to" (optional Filename.arg_type) ~doc:""
      in
      fun () -> run ~record_to ~write_to]
;;
